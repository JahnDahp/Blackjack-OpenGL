...
void ArrowButton::init()
{
  ...
  static std::shared_ptr<Texture> sharedTexture = std::make_shared<Texture>("../Resources/Textures/arrowbutton.png");
  shader = sharedShader;
...
void ArrowButton::transform(float x, float y, float scaleX, float scaleY)
{
  setPos(glm::vec2(x, y));
...
void ArrowButton::render(const glm::mat4& proj, const glm::mat4& view)
{
  shader->setView(view);
  shader->setProj(proj);
  if (isVisible()) getRenderer().draw(getVAO(), getIBO(), shader, GL_TRIANGLES, texture);
}
bool ArrowButton::tryButtonPressLeft(const glm::vec2& worldCoords)
{
  if (isVisible())
  {
...
bool ArrowButton::tryButtonPressRight(const glm::vec2& worldCoords)
{
  if (isVisible())
  {
    const glm::vec2& pos = getPos();
